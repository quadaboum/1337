...
def get_latest_users(limit=5):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT username, level, prestige FROM users ORDER BY created_at DESC LIMIT %s", (limit,))
            return [dict(username=u, level=l, prestige=p) for (u, l, p) in cur.fetchall()]

def get_averages():
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT AVG(level), AVG(prestige) FROM users")
            return cur.fetchone()
...
def dashboard():
    ...
    avg_level, avg_prestige = get_averages()
...
def update_user():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE users SET level = %s, prestige = %s WHERE username = %s",
                        (level, prestige, username))
            conn.commit()
    return redirect(url_for("dashboard"))
